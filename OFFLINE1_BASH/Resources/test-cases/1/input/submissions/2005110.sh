...
##match type , range 
#travarsing all the files/folder/zip from submission directory
make_evaluable()
{
    for file in *
do
    ...
    if [ $? -eq 0 ];then
        ...
        if [ $? -eq 0 ]; then ##file is in allowed format and language
            ...
            ##case 5 --not allowed range
            issueArr["$fileName"]="#case 5"
...
###make evaluable for false
make_evaluable_for_false()
{
    for file in *
do
    echo $file
...
report_generate()
{
    ...
    do
        ...
        while IFS= read -r line; do
            ...
            if [[ "$cid" == "$i" ]]; then
                issueArr[$i]="plagiarism detected"
                echo "copy------$cid"
...
initialize_all() {
    ...
    lineCount=$(grep -cve '^\s*$' "$filePath")
    if [ "$lineCount" -ne 11 ]; then
        echo "Error: The file must contain exactly 11 lines, but it contains $lineCount lines."
        exit 1
    fi
    ...
    while IFS= read -r line; do
        ...
        case $i in
            1) useArchieve="${fields[0]}"
                useArchieve=$(echo "$useArchieve" | tr -d '[:space:]')
                echo "useArchieve: $useArchieve"

                if [[ $useArchieve != "true" && $useArchieve != "false" ]]; then
                    echo "Error: Invalid value for useArchive. It must be true or false."
                    exit 1
                fi
                ;;


            2)  for arc in "${fields[@]}"; do
                    arc=$(echo "$arc" | tr -d '[:space:]')
                    if [[ -n "$arc" ]]; then  # Check if arc is non-empty
                        allowedArchieveFormats+=("$arc")
                    fi
                done
                echo "allowedArchieveFormats: ${allowedArchieveFormats[@]}"

                if [[ "$useArchieve" == "true" ]]; then
                    for format in "${allowedArchieveFormats[@]}"; do
                        if [[ "$format" != "zip" && "$format" != "rar" && "$format" != "tar" ]]; then
                            echo "Error: Invalid archive format $format. Allowed formats are zip, rar, tar."
                            exit 1
                        fi
                    done
                fi
            ...
            3)  for language in "${fields[@]}"; do
                    language=$(echo "$language" | tr -d '[:space:]')
                    if [[ "$language" == "python" ]]; then
                        language="py"
                    fi

                    if [[ -n "$language" ]]; then  # Check if arc is non-empty
                ...
                #check lang is in possible lang
                for lang in "${allowedLanguage[@]}"; do
                    if [[ "$lang" != "c" && "$lang" != "cpp" && "$lang" != "py" && "$lang" != "sh" ]]; then
                            echo "Error: Invalid language type"
                            exit 1
                        fi
                done
            ...
            4) totalMarks="${fields[0]}"
                ...
                if ! [[ "$totalMarks" =~ ^[0-9]+$ ]]; then
                    echo "Error: Total marks must be a valid positive integer."
                    exit 1
                fi
            ...
            5) penalty="${fields[0]}"
                penalty=$(echo "$penalty" | tr -d '[:space:]')
                if ! [[ "$penalty" =~ ^[0-9]+$ ]]; then
                    echo "Error: Penalty must be a valid non-negative integer."
                    exit 1
                fi
            ...
            7) 
                for sid in "${fields[@]}"; do
                    sid=$(echo "$sid" | tr -d '[:space:]')

                    if [[ -n "$sid" ]]; then  # Check if arc is non-empty
                        sidRange+=("$sid")
                    fi
                   
                done

                if ! [[ "${sidRange[0]}" =~ ^[0-9]+$ ]] || ! [[ "${sidRange[1]}" =~ ^[0-9]+$ ]] || [ "${sidRange[0]}" -gt "${sidRange[1]}" ]; then
                    echo "Error: Invalid student ID range."
                    exit 1
                fi
            ...
            8) outputFileLocation="${fields[0]}"
                outputFileLocation=$(echo "$outputFileLocation" | tr -d '[:space:]')
            ...
            9) penaltyForViolation="${fields[0]}"
                ...
                if ! [[ "$penaltyForViolation" =~ ^[0-9]+$ ]]; then
                    echo "Error: Penalty for submission guidelines violation must be a valid non-negative integer."
                    exit 1
                fi
            ...
            11) plagiarismPenalty="${fields[0]}"
                ...
                if ! [[ "$plagiarismPenalty" =~ ^100$|^0$|^[1-9]?[0-9]$ ]]; then
                    echo "Error: Plagiarism penalty must be a valid percentage (0-100)."
                    exit 1
                fi
...
main() {
    ...
    if [ -n "$inputPath" ]; then
        ...
        echo "folder submitted: ${folderSubmitted[@]}"
    else
